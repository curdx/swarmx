...
> - ⏳ **待做（远期，需 live ACP CLI 定 schema）**：ACP-specific session 层——`initialize` 握手 + 把 ACP notification（permission/tool-call/streaming）映射成 SwarmEvent + spawn.rs 的 piped-stdio 路径（现 transport seam 仍回退 PTY）。建在 `Connection` 上。
