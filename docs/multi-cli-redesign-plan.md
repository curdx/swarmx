...
> - ⏳ **待做**：把 bootstrap 的 `2500ms` 固定 sleep 换成对 MCP-ready banner 的 `wait_for`（机制可在 `spawn_bootstrap_inject` 里读 `slot.stream` 扫描，加超时兜底；但**banner 正则是 per-CLI 且版本相关，需真起一次 claude/codex 观察实际文案**才能定准 → 留待带真实 spawn 的验证）、`ready_plan` 的 `wait_for`/`input`/`extract_session_id`（顺序 onboarding）、L3 golden 测试、L4/L5。
