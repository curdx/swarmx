...
> - ✅ **L5 spawn 上限（修 fork-bomb F4）**：spawn 公共路径 `spawn_with_bookkeeping` 加**全局存活 agent 上限**（数 in-memory registry = 真并发上限，覆盖 /api/agent + /api/worker + run_spell）；`spawn_worker` 加**委派深度上限**（走 workers 表 parent 链，loop-bounded，终止于非-worker 的 orchestrator）。均 env 可调（`FLOCKMUX_MAX_LIVE_AGENTS=24` / `FLOCKMUX_MAX_SPAWN_DEPTH=6`），超限返回 429 + 可操作提示。实测 cap=2 时第 3 个 spawn 被 429 拒。依据 hermes MAX_DEPTH + openclaw 并发 lanes。
> - ⏳ **待做**：`ready_plan` 的 `wait_for`/`input`/`extract_session_id`（顺序 onboarding；现只有 answer_dialog）、L4 ACP 结构化传输、L5 余项（分层 registry override `~/.flockmux/cli-plugins` / model 与 CLI 解耦）。
