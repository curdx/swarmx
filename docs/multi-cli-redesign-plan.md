...
> - ⏳ **待做**：`ready_plan` 的 `wait_for`/`input`/`extract_session_id`（顺序 onboarding；现只有 answer_dialog）、L4 ACP 结构化传输、L5（分层 registry override / spawn 深度+fan-out 上限 / model 与 CLI 解耦）。
