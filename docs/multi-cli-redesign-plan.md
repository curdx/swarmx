...
> - ⏳ **待做**：`ready_plan` 的 `wait_for`/`input`/`extract_session_id`（顺序 onboarding；现 ready_plan 只有 answer_dialog）、L3 每 CLI golden 测试、L4 ACP 传输、L5（分层 override / spawn 上限 / model 解耦）。
