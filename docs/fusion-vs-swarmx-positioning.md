# swarmx vs. OpenRouter Fusion —— 思想定位与对外话术

> 调研日期：2026-06-17 ·  方法：官方文档逐条核对 + 多源联网调研 + 对抗式证伪（"同/异"双方辩护 + 裁决，裁决信心 0.85）+ 对照 swarmx 真实代码（`roles/orchestrator.md` 等）。
>
> 起因：有人问「OpenRouter Fusion 的思想跟 swarmx 是不是一回事？」——本文给出可对外引用的结论与依据。
...
- **swarmx = 异题分解（orchestration）**：队长把**一个目标**拆成不同子任务 → 分派给专长不同的 agent → 整合互不重叠的成果 → 产出**交付的真实软件**。

一句对外定位话术：

> **模型路由 / Fusion 类产品是「组合模型，把一个 prompt 答得更好」；swarmx 是「组合 agent，把不同的活分头干完并交付真实软件」。**
...
### swarmx 的血统 —— 编排 / 分工（把工作做完）

| 系统 | 怎么组合 | 引用 |
|---|---|---|
| **Microsoft Magentic-One** | 双 ledger（Task + Progress）编排器，按能力路由到专长 agent；stall→replan。**swarmx 直接照搬**（见下） | [MSR](https://www.microsoft.com/en-us/research/articles/magentic-one-a-generalist-multi-agent-system-for-solving-complex-tasks/) |
...
1. **都有一个不亲自干活、只负责合并的「特权协调者」**：Fusion 的 judge ≈ swarmx 的 orchestrator 收割那一步。形似。
2. **swarmx 确有一个 Fusion 形状的二级模式**：`orchestrator.md` 的「并行广度调研」——一次性 spawn N 个 worker 并行、下一轮收割综合。**拓扑同形（fan-out → synthesize）**——这是「同一思想」论点最强的地方。但**输入分布相反**：Fusion 给 N 个成员**同一个** prompt（冗余），swarmx 给 N 个 researcher **不同角度**、写不同 blackboard key（广度）；且它只是 triage 表里的一个分支，不是 swarmx 的身份。
3. **都用异构多厂商模型**——但动机不同（多样性聚合 vs 专长分工）。
4. **可组合**：Fusion 完全可以当成 swarmx 某个 worker 内部那一次推理调用的模型端点。能干净嵌套，说明确实共享一条概念边界。
5. **聚合是「原语 / 手法」而非身份**：编排框架（含 swarmx orchestrator、甚至 Claude Code 的 Workflow "judge panel"）随时可以把一次 Fusion 式聚合塞进某一步当工具用——所以「会用聚合」不等于「就是 Fusion」。
...
1. **模型路由 routing**（选 1/N）：NotDiamond / RouteLLM / OpenRouter Auto——*两者都不是*（swarmx 仅在「每个 worker 选哪个引擎」处带一丝路由味）。
2. **集成 / 聚合 ensemble**（同题多答再合并）：**Fusion 在此**（ensemble-after-inference / MoA）。
3. **编排 / 分工 orchestration**（异题分解再整合）：**swarmx 在此**（Magentic-One 同范式）。
...
- 一句话：**「Fusion 组合模型把一个答案答得更好；swarmx 组合 agent 把不同的活分头干完并交付软件。」**
...
**对照的本仓代码**
- `roles/orchestrator.md`（Magentic-One 双 ledger、拆解—委派、类型化 handoff）· `crates/swarmx-server/src/{wake,roles,spells}.rs`（依赖唤醒 / 类型化 spawn / ledger）· `README.md`（架构总览）
