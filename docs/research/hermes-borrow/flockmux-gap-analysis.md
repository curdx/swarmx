# flockmux × hermes 逐主题对比：哪些真的需要（带代码证据, 2026-06）

> 方法：5 个核查 agent **真打开 flockmux 代码**逐主题核实"到底有没有、缺不缺"，不是臆测。证据均为 `crate/文件.rs:符号` 或 `web/src/...tsx`。
> 与 `flockmux-borrow-roadmap.md` 的区别：那份的"flockmux 现状"是基于记忆的假设；本份是基于真代码——**有几处把 roadmap 的假设推翻了**（见末节"roadmap 修正"）。
> 判定图例：✅已有充分 · 🟡有地基可增量 · 🔴真空白需新建 · ⚪不适用
>
> **深度复核修正（2026-06，作者亲自逐文件读码）**：18 条承重论断 17 条对源码确认无误；**1 条推翻**——"worker env allowlist 比 hermes 更严、不漏父环境"是**错的**。真相：`flockmux-pty/src/lib.rs:76` 用 `CommandBuilder::new`，而 portable-pty 0.9 的 `CommandBuilder::new`（`cmdbuilder.rs:218 envs: get_base_env()`）用 `std::env::vars_os()`（**整个父环境**）做种子，且 flockmux **全仓无 `env_clear()`**——所以 worker **继承 flockmux-server 的全部环境变量**（含你导出的 `ANTHROPIC_API_KEY`/`OPENAI_API_KEY`/代理变量）；`spawn.rs:210` 那段"allowlist"只是叠加 override，注释写的"Drop everything else from the parent" **并未实现**。→ hermes 的"PTY env allowlist 剥 server token"借鉴点 **是真需要的**，且与本项目 per-agent 凭证隔离目标（见 CODEX_HOME 隔离）直接矛盾。修法见 §2 quick-win。

---

## 0. 一图速览

| # | 主题 | 判定 | flockmux 现状（证据） | 真缺口（一句话） |
|---|---|---|---|---|
...
| 4 | cron / 定时编排 | 🔴 | 无 scheduler；migration 无 cron 表（`CronCreate` 是 Claude Code 工具非 flockmux 功能） | 无定时 spawn orchestrator 能力 |
...
| 10 | 文件路径安全 + 原子写 | 🟡 | `flockmux-swarm/src/path_safe.rs` 完整（lexical+canonicalize+starts_with，有测试）；config 写已 tmp→rename | **blackboard 写非原子（`swarm.rs:288` 直接 fs::write）**；无 per-path 锁 |
...
| 18 | 把 flockmux 暴露成 MCP server | 🟡 | 内向 MCP 已全：`flockmux-mcp/src/tools.rs:55` 暴露 10 个 `swarm_*` 给 worker；`pre_spawn.rs:712` 自动注入 | 缺外向 endpoint（给外部 Claude Code/IDE 用） |
...
- **抄**：状态机 + circuit breaker → hermes-agent `kanban_swarm`；Kanban UI（7 态 + WS 实时 + 终态保护 + 两步 complete/block）→ hermes-web-ui `KanbanView`；diagnostic rules（无状态只读规则）思路同 flockmux harness-check。
...
- **现状**：无任何压缩。注意 **PTY 下 worker 自身上下文由 claude/codex CLI 自管（有原生 /compact），flockmux 不需要碰**。真空白在 orchestrator 汇总层：读黑板 + 给 worker 分发 + 拼 system prompt，worker 一多 orchestrator 自己 overflow。
...
| 增量点 | 现状基线 | 要补的那一刀 | 量级 |
|---|---|---|---|
| **worker env 隔离（安全真缺口）** | spawn.rs 已构造干净 env map，但 portable-pty 默认继承父全环境、无 `env_clear` | `flockmux-pty/src/lib.rs:76` 建完 `CommandBuilder` 后调 `cmd.env_clear()` 再 apply allowlist；同时把 allowlist 补全（claude/codex 真正需要的：HOME/PATH/LANG/TERM/CODEX_HOME/FLOCKMUX_*，按需加 HTTPS_PROXY/NO_PROXY/NODE_*）。**实现 spawn.rs 注释里已声明的意图**，与 per-agent 凭证隔离目标一致 | quick win（一行 + 调 allowlist） |
| **blackboard 原子写** | `path_safe.rs` 路径安全已全；config 已 tmp→rename | `flockmux-swarm/src/swarm.rs:288` 的 `fs::write` 改 tmp→rename（黑板是最关键持久态，crash 会写出截断文件） | quick win |
...
这几项 hermes 借鉴清单里看着诱人，但**代码证据显示 flockmux 核心已做**，只在特定场景才需补：
...
上一份 `flockmux-borrow-roadmap.md` 基于记忆假设，把这几项列得过高/过宽，**真代码核实后应下调或收窄**：
...
4. **实时活动渲染** flockmux 已有完整 running→done+seq 链路，hermes 的只是语义摘要增量——确认是 P1 质量增量非新建。
