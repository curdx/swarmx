...
>
> 目标：为 flockmux「F1 角色/任务感知配给」找设计依据——让「哪个角色 + 哪个 CLI + 哪个模型 接哪个任务」从**拍脑袋猜**变成**系统化/能力驱动**。
...
>
> 对 flockmux，最高置信、最广共识、最低风险的两件事是 **P0**：
...
- **取舍**：最简单、可 diff、零路由延迟；但**无法把难任务升级到强模型、也无法把琐碎任务降级** —— 这正是 flockmux **不能照抄**的缺口。Aider 证明：static 可以接受，**前提是能力以数据声明而非靠猜**。
...
- **取舍**：职责清晰（worker 专注能力、manager 拥有配给+校验+重试）；MagenticOne 的双 ledger + 停滞检测是「`.done` 永远没来」的原则性答案。但**星型每轮 round-trip 一个 LLM**——并行的 token/延迟瓶颈，**与 flockmux 并行扇出黑板模型不合**。
...
- **取舍**：两全——80% 便宜确定、尾部智能，各层可换可基准。**这就是 flockmux 该建的**。代价：更多活动件、两份契约要保持一致。**每个成熟的 router-class 仓库最终都收敛到这里。**
...
9. **订阅即契约 / 按声明类型 pull 的 pub-sub** —— **最贴 flockmux 黑板拓扑**。消费者声明它 watch 的输出**种类**，key 从注册种类算出，绝不手打。→ MetaGPT `_watch([ActionClass])` over message pool；conductor worker 轮询它实现的精确 task-type。
...
1. **脆弱自由文本 handoff/收件人/key 契约，静默或迟到失败**。**flockmux 已中招**（`depends_on`/`handoff_signal` typo/前缀/大小写漂移破坏 WakeCoordinator，见 docs F3）。处处可见：plano route 名不符 warn-and-drop；NVIDIA `MODEL_ROUTER_TO_TARGET` 重复 key bug + 子串模糊重映；claude-code-router 拼错模型名静默落默认；LLMRouter `_parse_model_name` 模糊猜 + 落第一个模型。**模糊归一化是症状不是解药。**
...
4. **星型拓扑** —— 每个 worker 每轮 round-trip 中央 manager LLM → 并行的 token+延迟瓶颈，**结构上不合 flockmux 并行扇出黑板**。langgraph-supervisor 维护者自己都在转向。
5. **模型钉在 ROLE 上、无 per-task 分层** —— 无法升级难任务/降级琐碎任务。**这正是 flockmux 必须补的维度。**
...
8. **隐式完成信号（"发布输出然后祈祷"）静默卡死依赖方**。flockmux 已学到（Stop-hook/WakeCoordinator）。MetaGPT 经典完成 = 「我发布了 Action 输出」（漏发布卡死 watcher）。
...
- 内建默认写在 Rust，项目级 `.flockmux/roles.toml` 按 slug 扩展/覆盖，坏条目 soft-fail（精确记 issue、跳过）+ 生成 editor JSON-schema + 漂移测试。`swarm_spawn_worker` 选校验过的 slug，未知 slug 大声报错。
...
- **解决 #3**。门控比分层更重要，因为 **flockmux 的 CLI 是非对称的**（codex app-server vs Claude PTY、MCP per-agent 配置、vision/长上下文支持不同）→ 资格是硬约束。**effort 是 L 而非 M**：per-(cli,model) 粒度 + 底座非对称使其不止是模型层表。Aider 证明 per-MODEL 卡只在底座统一（litellm）时够用——flockmux 底座不统一，故需 per-(cli,model)。借鉴：RooCode 工具组+fileRegex（门控机制）、Aider 能力卡、litellm `model_info`、langflow `ModelMetadata`、lmos-router、semantic-router modelCards。
...
- **攻击 #1 成本维度**。典范生产实例是 **Aider architect→editor**（critic gap #4，正是 flockmux 域）。借鉴：Aider、MetaGPT TL t-shirt sizing、ruflo Tier-0 WASM codemod($0)、plano、BMAD。
...
1. **modality 不可靠推断**：taxonomy 与建议曾倾向把 modality 当干净路由信号，但**无仓库可靠从自由文本提取它**。→ flockmux 应把 modality 当 **spawn 时声明的 orchestrator 输入**，规则键控可抽取代理（code-fence/shell/文件路径密度）。代价：把猜推到 orchestrator 这一层。
...
5. **拓扑错配（已承认）**：flockmux 是并行扇出 + 共享黑板 + 事件驱动 WakeCoordinator；样本绝大多数是星型/顺序/设计期静态 DAG。最近拓扑的是 MetaGPT `_watch` pub-sub 与 conductor pull-by-type 队列，但**两者都不在并行黑板 swarm 上做 LLM 驱动的 role/cli/model 路由**。→ 建议是把路由层**外推**到 flockmux 拓扑，而非照抄已验证的同类。
...
6. **离线语料可得性**：flockmux 有足够带成功标签的历史 swarm 跑来训练，还是必须先上 P2-B instrumentation 等数据？
...
9. **角色爆炸 vs 复用**：ruflo 98+ 角色定义漂移；BMAD/OpenHands 保持小而精。flockmux 内建注册表多大合适，怎么防 `.flockmux/roles.toml` 覆盖碎片化成 per-project 漂移？
...
- 对模型/CLI/成本/延迟**只字未提**——不解决 flockmux 的 per-task 分层问题。
