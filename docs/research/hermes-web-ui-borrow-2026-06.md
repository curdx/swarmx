...
> 方法：4 个并行子代理分别深挖「多 agent 协同/chat 引擎」「可观测/模型/cron/profile」「前端 UX/设计系统」「基础设施/架构/工程化」四个维度，读实际代码并对照 swarmx 评估可搬性。
> 目的：从 Hermes Web UI 提炼可搬到 swarmx 的设计与功能。
...
**与 swarmx 的关系**：同域（AI agent 的 Web 管理 UI），但定位互补——
- Hermes Web UI = 单 agent 的**全功能仪表盘**（用量分析、模型发现、cron、Web 终端、文件浏览、日志、多 profile、IM 渠道）。
- swarmx = 多 CLI agent 的**编排/协同**（swarm、blackboard、worktree 隔离、typed handoff、WakeCoordinator）。
- 因此 Hermes 的「外围能力」（可观测、终端、设置体验、工程化）恰好是 swarmx 的短板，值得搬；而 Hermes 的「单 agent chat 引擎」与 swarmx 的「多 worker 编排」内核差异大，多为思路参考。

**关键前提**：Hermes 底层 CLI 会吐结构化 `estimated_cost_usd`、自带 cron 调度、管 profile；而 swarmx 底层是 claude/codex CLI，**不吐成本、不带调度、不管 profile**。所以 Hermes「委托给 CLI」的部分，swarmx 大多得自建；但其「消费层/工具层」（聚合 SQL、价格源、URL 探测、纯 CSS 图表、文件化历史、机械化校验）可大量直搬。
...
| 2 | **Usage/Cost 可观测全链路**：采集→聚合 SQL→models.dev 价格表→纯 CSS 图表 | swarmx 当前完全空白；程序员强需求 | 中（采集层需自建） |
| 3 | **实时活动面板 + 工具调用折叠行 + context_status 进度**：让 worker "正在干嘛"可见 | 直击 swarmx 可观测性核心；体验提升最大 | 中 |

### P1 — 推荐（高价值，体验/质量明显提升）

| # | 项目 | 价值 | 落地成本 |
|---|------|------|---------|
| 4 | **Web Terminal（旁观/接管 worker PTY）**：单 WS 多会话 + 首字节分流协议 + xterm 接入 | 程序员想直接看 worker 终端；swarmx 已有 PTY | 中（前端直接可搬，Rust 后端复刻协议） |
...
| 8 | **代码块复制 + diff 折叠渲染**（`highlight.ts` 纯函数） | swarmx 是 coding agent 工具，展示 diff 是核心场景 | 低（纯函数可移植） |
...
- 对 swarmx：PTY 模式 CLI 自管上下文，不直接适用；但 blackboard/message 流无限增长，orchestrator 汇总进展时正缺此机制。**移植算法到 Rust**，契合 per-CLI 模型配给（用廉价 aux 模型摘要）。
...
- 对 swarmx：把 worker 生命周期（spawn→建上下文→运行→写 blackboard→done）做成 typed status 广播，join/重连带快照。**直接可搬思路，提升 swarm 可观测性**。
...
- cursor-based bridge 可恢复流（`agent-bridge/client.ts:462-494`）：双游标（cursor + event_cursor）天然可恢复，文本 delta + 结构化 events 双流。swarmx 未来接 Codex app-server 时值得重提。

---

## 维度二：可观测 / 模型管理 / cron / profile

### Usage/Cost 可观测（swarmx 空白，重点）
- **采集**：`proxy-handler.ts:119-186` 边转发 SSE 边解析，命中 `run.completed` 写 `data.usage`；`usage-store.ts:15-51` 写 SQLite（带 JSON fallback）。字段：input/output/cache_read/cache_write/reasoning + model + profile。
  - swarmx 改造：claude `--output-format stream-json` / codex JSON 事件流逐事件抓 `usage.{input,output,cache_creation,cache_read}_tokens`，写一张 SQLite usage 表（加 `agent_id`/`thread`/`role` 维度）。
...
- **价格表数据源**：`model-context.ts:10` 用 `models_dev_cache.json`（opencode 同款 models.dev 数据集，含每模型 context/input/output/cache 单价）。swarmx 可定期拉 `models.dev/api.json` 或打包静态快照。
...
- OAuth 设备流（RFC 8628）：`copilot-device-flow.ts` start/poll/status 三段式。**swarmx 不适用**（登录由 CLI 自管）。
...
- swarmx：调度核心需自建（Rust cron + tokio timer）；可搬产物文件化 + synthetic 补洞 + 预设表单。注意融入 swarmx CronCreate 已有的「整点错峰」理念。
...
- 对 swarmx：印证 per-agent CODEX_HOME / `--mcp-config` 隔离思路；profile 可作比 thread 更上层的「凭证集」隔离。
...
- `scripts/harness-check.mjs`（379 行纯 Node 零依赖）把"只能靠人记的规矩"变 CI 断言：
  ...
  6. 失败收集全部一次性打印。
- **swarmx 落地**：把 MEMORY.md 每条坑写成断言——grep `tauri.conf.json` 确认 server+shim+mcp 三 binary 都在 externalBin、grep lib.rs 确认新 model 有 re-export、diff-aware 检查「改了 storage::MessageRecord 是否同步改 protocol::rest::MessageRecord」、检查 codex CODEX_HOME 隔离逻辑存在、「改 chat 链路要更新文档」。Rust xtask 或 .mjs 均可，挂 CI 必跑。
...
- swarmx：前端纯代码直接可搬；Rust 后端用 `portable-pty`+`tokio-tungstenite` 复刻，协议原样照搬。「点开某 worker 看/接管实时 PTY」是高价值可观测性。
...
- swarmx：多后端是过度设计（local 一种够用），但**统一 trait + 错误码归一化 + provider 工厂 TTL 缓存**值得照抄，为未来"worker 跑远程/容器"留扩展点。
...
### Bridge Broker 状态机（与 swarmx 核心同构）
...
- swarmx 可搬：readiness 双信号（直击 codex 启动卡死/503）、退避+曾就绪才重启门槛、日志 summarize、可选串行化锁。
...
- swarmx：桌面单机用「随机 token 写文件 + 127.0.0.1 only」即可；Web/LAN 模式时三层限流是现成参考。

### 桌面 sidecar 启动（swarmx 踩过的坑的正解对照）
- `webui-server.ts`：
...
### 可下载运行时 + CI release matrix（多数不适用 swarmx）
- runtime 不打进安装包，首次启动从 CDN 下载按 sha256 校验；electron-builder filter `!node_modules/node-pty/prebuilds/!(${platform}-${arch})/**` 只留当前平台 prebuild。swarmx 三 Rust binary 体积本就小，不适用；仅借鉴「只留当前平台 prebuild」「机械化断言禁止打不该打的」理念。
...
- swarmx 因底层是 claude/codex（不吐成本/不带调度/不管 profile），**采集层与调度核心要自建**；但 Hermes 的**消费层/工具层**（聚合 SQL、models.dev 价格源、URL 探测、纯 CSS 图表、文件化历史、机械化校验、xterm 接入、防穿越、readiness 状态机）可大量直搬或借鉴。
