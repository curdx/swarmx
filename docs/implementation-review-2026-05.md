...
- **其他**：~~`Shell.tsx` 1439 行 god-file~~（✅ 已拆：UI→WorkspaceSidebar/Toolbar、数据→useWorkspaceShellData hook，现 ~300 行）；~~`main.tsx` 全局关 StrictMode~~（✅ 已重新启用，M3 broadcast 多连接 + XtermPane disposed 守卫已就绪）；per-CLI 知识硬编码（✅ input_settle 已数据化、GraphPanel 已删）。**eslint-disable / any 审计结论**：核实后 `as any` 仅 2 处（OnboardingTour 挂 window，合理 escape）；`exhaustive-deps` 15 处**全部确认为有意的 escape hatch**——都是 `[<key>]` / `[]` 的"只在某 key 变化时 re-run"模式（callback 走 ref 读最新值），盲加 deps 会造成 refetch 循环；几处已带注释，给共享的 `useSwarmFeed` 补了详注。`no-console`(26) 多为诊断日志，留作后续统一日志框架。结论：属代码卫生非缺陷，无行为变更必要。

---

## P3 — spells / roles

- **spells/roles = deliberately minimal（✅ 已决策，2026-05）**：磁盘上只有 1 spell（init）+ 1 role（orchestrator）。`role_ref` 合并、`system_prompt_prefix`(HITL)、`allow_cycles`、`{<role>_id}` 互引、shared_workspace、多 agent spawn 全实现+单测，但只有 `init`（单 orchestrator，建 workspace 时经 `run_spell` 用）被在线驱动；下游全走 `swarm_spawn_worker` ad-hoc 派活（Magentic-One：按任务定团队、不预设拓扑），`swarm_run_spell` MCP 工具已移除。**核实结论：不是腐烂的 dead-weight，是有意精简且被 init 实际使用。决定：保留不动**（删会断 init + 丢已测机制；补 speculative spell 是负担）。已在 `spells.rs`/`roles.rs` 头记录"deliberately minimal、勿删勿乱补"的意图，并清掉 README 里"5 个法术 + swarm_run_spell"的陈旧清单。README 的 M6c auto-dispatch 叙述属历史，未逐句改写。
