...
> 一次性、全面的 swarmx 核心性能审计。覆盖前端（React/Vite/xterm）、后端（axum/PTY/tokio）、
...
| 后端 | `crates/swarmx-{server,pty,shim}` | 同上 |
| 存储 | `crates/swarmx-{storage,swarm}`（SQLite+FTS5） | 同上 |
| 后台循环 | `swarmx-server` 的 wake/reaper/billing/probe/cron | 同上 |
...
**改动**：`crates/swarmx-storage/migrations/0025_message_perf_indexes.sql`（新）+ `schema.rs` 注册。
...
**验证**：`cargo test -p swarmx-storage` 44 测试绿（含 `fresh_db_migrates_to_latest`=25、
...
这正是「跑得越久越卡」的典型签名——而 swarmx 的核心场景就是长时间挂着的 swarm 会话。
...
| 文件 | 改动 |
|---|---|
| `crates/swarmx-storage/migrations/0025_message_perf_indexes.sql` | 新增：2 个索引 + 版本记录 |
| `crates/swarmx-storage/src/schema.rs` | 注册 MIGRATION_0025 |
| `crates/swarmx-storage/src/store.rs` | 连接池 `max_size(8)` → `16` |
...
| 验证 | 命令 | 结果 |
|---|---|---|
| 存储层测试（含迁移幂等/最新版本=25） | `cargo test -p swarmx-storage` | ✅ 44 passed |
