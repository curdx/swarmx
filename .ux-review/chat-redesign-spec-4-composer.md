...
实现锚点主体：`/Users/wdx/opc/swarmx-core/web/src/components/MessagesPanel.tsx`（Composer 渲染块 `:1437-1565`，逻辑 `:258-1023`）。对齐文档：`chat-redesign.md §4.4`（`:152-170`）、原则 6「可打断可操控」（`:41`）。
...
实现起点文件（绝对路径）：
- `/Users/wdx/opc/swarmx-core/web/src/components/MessagesPanel.tsx`（Composer 全部逻辑 + 渲染）
- `/Users/wdx/opc/swarmx-core/web/src/components/ModelPicker.tsx`（包确认弹窗）
- `/Users/wdx/opc/swarmx-core/web/src/api/http.ts`（`interruptAgent:229` / `interruptAllInWorkspace:239` / `uploadAttachment:361` / `optimizePrompt:351`）
- `/Users/wdx/opc/swarmx-core/web/src/lib/agent.ts`（`roleColorHex:47` / `resolveRole:68`）
- `/Users/wdx/opc/swarmx-core/web/src/lib/platform.ts`（`getClientPlatformInfo:33`）
- `/Users/wdx/opc/swarmx-core/web/src/i18n/locales/zh.json` / `en.json`（新增 `messages.*` key）
- `/Users/wdx/opc/swarmx-core/web/src/styles/global.css`（token 来源，勿改值）
- 后端 emit 点：`/Users/wdx/opc/swarmx-core/crates/swarmx-storage/src/store.rs:1070`（messages kind+meta）、`/Users/wdx/opc/swarmx-core/crates/swarmx-server/src/routes/rest.rs:1806/1861`（interrupt）
