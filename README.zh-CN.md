...
flockmux 给每个工作空间一个统一入口：一个 **orchestrator** agent（claude），
...
2. 打开 web UI，创建一个指向真实项目目录的工作空间。flockmux 会跑内置的
...
每一次手把都是架构驱动的——agent 的 Stop hook fire 出 `flockmux-mcp wake-check`，
