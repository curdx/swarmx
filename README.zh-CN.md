...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/状态-M1–M6b%20已交付-success" alt="status">
...
- [演练: ~9 分钟跑 fullstack-feature (M6)](#演练-9-分钟跑-fullstack-feature-m6)
- [架构](#架构)
- [配置参考](#配置参考)
...
| **Turn 边界 wake-check** | claude 和 codex 的 Stop hook 都调 `flockmux-mcp wake-check`，未读 > 0 时 hook 返回 `decision:block` 让 agent 续 turn 读消息——0 轮询。 |
...
| **Wake-check** | `flockmux-mcp wake-check` 子命令。从 Stop hook 的 stdin JSON 解析 `agent_id`（优先用 spawn 注入的 `FLOCKMUX_AGENT_ID` env，fallback 才是 cwd basename），调 `/api/message/unread_count`，未读 > 0 就输出 `{decision:"block", reason:"..."}`。**单次性**——只在 Stop 那一刻触发，不能唤醒已停止的 agent（那是 WakeCoordinator 的活）。`~/.flockmux/wake/<id>.json` 文件做滑动窗口节流。 | `flockmux-mcp::wake_check` |
| **Spell** | `spells/<name>.md`，TOML front-matter 声明 `[[agents]]`。每个 agent 块可以内联 `role/cli/system_prompt` 或者用 `role_ref="<id>"` 从 `roles/<id>.md` 模板继承。`shared_workspace = true` 把 spawn 从"每 agent 一个目录"切到"共享同一个 cwd"。`POST /api/spell/run {name, task, workspace_dir?}` 拉起。 | `spells/`, `flockmux-server::spells` |
| **Role** | `roles/<id>.md`——可被多个 spell 复用的 SOP 模板。带 `default_cli`、`artifact_paths`、`handoff_signal`、`depends_on` 和 `system_prompt_template`（含 `{task}` / `{<role>_id}` 占位符）。多个 spell 共用同一份 FE/BE/test prompt 不用复制粘贴。 | `roles/`, `flockmux-server::roles` |
| **`depends_on` + WakeCoordinator（M6b）** | 角色声明它订阅哪些黑板 key。spell 启动时 `register_wake_subs` 在 `AppState` 里建 `Map<agent_id, Vec<key>>`。`WakeCoordinator` task 订阅 `Swarm::events_tx`，遇到 `BlackboardChanged{key}` 就给所有订阅者（排除 writer 自己）写一条 `kind="wake"` mailbox note 并往他们 PTY 注入 `\x15<msg>\r`。spawn 前先 BFS 检测 `depends_on` 循环。 | `flockmux-server::wake` |
| **Shim** | `flockmux-shim`，~70 行二进制。`execvp` 真实 CLI 同时发 OSC `ready` / `exit` 序列，让 flockmux 0 轮询就能检测生命周期。 | `flockmux-shim` |
| **MCP** | `flockmux-mcp`，stdio JSON-RPC 服务，给 LLM 暴露 `swarm_send_message` / `swarm_list_messages` / 黑板工具。pre-spawn 自动写到每个 agent 的 CLI 配置里，LLM 把它们当原生 tool 用。claude agent 额外拿一份独立的 `--mcp-config` 文件 `~/.flockmux/mcp/<agent_id>.json`，避免共享 workspace 下 `~/.claude.json` 里身份串台。 | `flockmux-mcp` |
...
## 演练: ~9 分钟跑 fullstack-feature (M6)

`fullstack-feature` 是把架构推到 M6 形态的那个 spell：3 个 agent 共享同一
monorepo，靠黑板契约做 fork-join。
...
# 2. 准备一个干净的共享 workspace
mkdir -p /tmp/m6-todo && cd /tmp/m6-todo && git init -q

# 3. 拉起 spell（UI 下拉菜单点 run 触发的是同一个 API）
curl -sX POST http://127.0.0.1:7777/api/spell/run \
  -H 'content-type: application/json' \
  -d '{
...
接下来 ~9 分钟里服务端日志会按顺序吐这些行（实际数字来自 commit
`228c260` 的 demo run）:

```
00:00  3 个 agent spawn，每个 claude agent 拿到独立 --mcp-config 文件
       ~/.flockmux/mcp/<agent_id>.json（绕开 ~/.claude.json 共享 cwd 串台）
00:40  BE 把 api.spec 写到 ~/.flockmux/blackboard/
       → WakeCoordinator: "wake delivered target=FE-id key=api.spec"
04:07  FE 写完 apps/frontend/ + commit + 写 frontend.done
       → WakeCoordinator: "wake delivered target=test-id key=frontend.done"
05:22  BE 写完 apps/backend/ + commit + 写 backend.done
       → WakeCoordinator: "wake delivered target=test-id key=backend.done"
08:44  test 装 Playwright + 写 tests/ + 跑 14 个测试全绿 + commit + 写 test.passed
```

跑完后 `/tmp/m6-todo` 里：
- `apps/frontend/` — React 18 + TS + Vite + Tailwind，`npm run dev` 直接跑
- `apps/backend/` — FastAPI + SQLite + uvicorn，`uvicorn main:app` 直接跑
- `tests/` — 14 个 Playwright 测试覆盖 4 个接口 + UI smoke
- 4 个 git commit（FE 2 + BE 1 + test 1）
- 4 个黑板 key：`api.spec` / `frontend.done` / `backend.done` / `test.passed`

整个 handoff 100% 是架构推动的——没有 role 看时钟、没有 role 轮询、没有
人手戳 PTY。每条 `wake delivered` 日志都是 `WakeCoordinator` task 在另一个
agent 调 `swarm_write_blackboard` 那一瞬间的副作用。
...
| `flockmux-server` | ~5700 | axum HTTP/WS gateway。路由、生命周期、pre-spawn 补丁、dialog 自动应答、spell 执行器、角色注册表、**`WakeCoordinator`**（M6b）。 |
| `flockmux-swarm` | ~600 | 每 agent 收件箱、黑板 CRUD、notify-debouncer watcher。 |
| `flockmux-mcp` | ~2000 | stdio JSON-RPC MCP server。也是 Stop hook 调用的 `wake-check` 子命令的宿主。 |
...
| `roles/` | — | 每个角色一个 `.md` SOP 模板：`frontend.md`、`backend.md`、`test.md`（M6a）。 |
| `spells/` | — | 每个法术一个 `.md`：`critic-loop.md`、`fullstack-feature.md`。 |
...
| 优先级 | 内容 | 工作量 |
|---|---|---|
| P1 | M6c — Planner agent：自然语言任务 → 派发 spell + 参数 | 新角色 + spell 加载器暴露 metadata |
| P1 | M6c — 嵌入 critic-loop：每个 handoff 后过一遍 critic | 新角色 + spell 拉线 |
...
| P2 | M6c — architect 阶段后 / test 阶段前的 HITL 卡点 | spell manifest 加 flag + UI approval 按钮 |
| P2 | M6c — `<key>.failed` TTL 兜底（依赖一直没到自动写 failed） | wake.rs ~40 行 |
| P2 | M6c — swarm drawer 里画 `depends_on` DAG | 前端 SVG + 从 `/api/spells` 读元数据 |
| P2 | M6c — agent 在 thinking 时跳过 PTY 注入 | 跟踪每 PTY 的状态（OSC + stop hook） |
