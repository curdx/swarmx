...
# M6b: FE waits on backend's api.spec contract. The WakeCoordinator
# auto-wakes us the moment BE writes it, so we don't have to rely on
# BE remembering to send a notification message (M6a race: FE stops
# before that message arrives, never wakes).
depends_on = ["api.spec"]
