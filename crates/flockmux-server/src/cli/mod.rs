...
//!
//! flockmux drives several coding CLIs — claude, codex, opencode — through one
...
pub struct PreSpawnCtx {
    ...
    pub agent_id: String,
    /// Absolute path to `flockmux-mcp` — baked into the per-spawn MCP entry so
    /// the path doesn't drift with the user's CWD or PATH.
    pub mcp_bin: PathBuf,
    /// Base URL of the flockmux-server REST API the MCP subprocess will talk to.
