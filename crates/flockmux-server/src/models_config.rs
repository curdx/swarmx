...
//!
//! Persisted as `~/.flockmux/models.json`, edited via the 模型 settings page
...
//! model overlay always was — flockmux-local, not read from ~/.claude/~/.codex).
...
/// `~/.flockmux/models.json`.
pub fn models_config_path() -> PathBuf {
    if let Ok(home) = std::env::var("HOME") {
        return PathBuf::from(home).join(".flockmux").join("models.json");
    }
    PathBuf::from(".flockmux/models.json")
}
...
/// Atomically persist to `~/.flockmux/models.json` (temp → fsync → rename),
