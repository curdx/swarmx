...
//! flockmux thesis is the opposite: run the UNMODIFIED CLI interactively over a
...
pub async fn claude_one_shot(
    ...
    let tmp = std::env::temp_dir().join(format!("flockmux-optq-{}", &Uuid::new_v4().to_string()[..8]));
...
mod tests {
    ...
    fn read_assistant_text_missing_file_is_none() {
        assert!(read_assistant_text(Path::new("/no/such/flockmux/transcript.jsonl")).is_none());
    }
