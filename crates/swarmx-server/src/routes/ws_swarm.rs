...
//! (1024, see `Swarm::new` in `swarmx-swarm/src/swarm.rs`) is
...
async fn handle(socket: WebSocket, state: AppState) {
    ...
    // swarmx::msg send line). INFO so it shares the default-filter timeline.
    tracing::info!(target: "swarmx::ws", "ws/swarm subscriber attached");
        ...
        tokio::select! {
            ...
            recv = rx.recv() => match recv {
                Ok(ev) => {
                    ...
                    if let swarmx_swarm::SwarmEvent::Message { id, from_agent, to_agent, .. } = &ev {
                        tracing::info!(
                            target: "swarmx::ws",
                            id = *id,
                            from = %from_agent,
