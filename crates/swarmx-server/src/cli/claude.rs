...
/// The `swarmx-swarm` MCP entry for one agent. Shared by the local-scope
...
/// Mark `swarmx-swarm` as a local-scope MCP server in
...
/// `args ["--agent-id", "..."]` and `env { SWARMX_AGENT_ID: "..." }`. If the
/// user later runs `swarmx-mcp` by hand the env wins; if claude clears the
...
fn patch_claude_mcp_at(
    ...
    if mcp_servers.get("swarmx-swarm") == Some(&desired) {
        return Ok(());
    }
    mcp_servers.insert("swarmx-swarm".into(), desired);

    write_json_atomic(cfg, &root)
}

/// Write a workspace-local `.claude/settings.local.json` carrying a Stop
/// hook that calls `swarmx-mcp wake-check`.
///
...
/// inside the swarmx-managed workspace anyway.
...
/// Per-agent claude MCP config file. Written under `~/.swarmx/mcp/` keyed
...
fn write_per_agent_mcp_config(agent_id: &str, mcp_bin: &Path, server_url: &str) -> Result<PathBuf> {
    ...
    // `mcpServers` block in here, then force-overwrite swarmx-swarm with THIS
...
/// plus a guaranteed-correct per-agent swarmx-swarm. Pure (no IO) so the merge
/// rule is unit-tested. Any `swarmx-swarm` from the user scope is dropped first
...
mod tests {
    ...
    fn per_agent_body_inherits_user_servers_and_overrides_swarm() {
        let mcp_bin = std::path::Path::new("/opt/swarmx-mcp");
        ...
        // swarmx-swarm is force-overwritten with THIS agent + THIS binary,
        // never the stale OTHER-AGENT entry.
        assert_eq!(
            servers["swarmx-swarm"]["command"],
            json!("/opt/swarmx-mcp")
        );
        assert_eq!(
            servers["swarmx-swarm"]["args"],
            json!(["--agent-id", "agent-A"])
        );
        assert_eq!(
            servers["swarmx-swarm"]["env"]["SWARMX_AGENT_ID"],
            json!("agent-A")
        );
    }
    ...
    fn per_agent_body_with_no_user_servers_has_only_swarm() {
        ...
        assert!(servers.contains_key("swarmx-swarm"));
    }
    ...
    fn read_user_scope_takes_top_level_not_project_scope() {
        ...
        fs::write(
            ...
            serde_json::to_vec(&json!({
                ...
                // per-workspace swarmx-swarm with a possibly-foreign agent id.
                "projects": {
                    "/some/ws": {
                        "mcpServers": {
                            "swarmx-swarm": { "command": "/x", "args": ["--agent-id", "OTHER"] }
                        }
                    }
                }
            }))
    ...
    fn claude_mcp_local_writes_new_entry() {
        ...
        let written: Value = serde_json::from_slice(&fs::read(&cfg).unwrap()).unwrap();
        let entry =
            &written["projects"][ws.to_string_lossy().as_ref()]["mcpServers"]["swarmx-swarm"];
        ...
        assert_eq!(entry["env"]["SWARMX_AGENT_ID"], json!("claude-aaa"));
        assert_eq!(
    ...
    fn claude_mcp_local_preserves_other_mcp_servers() {
        ...
        assert_eq!(
            mcp["swarmx-swarm"]["env"]["SWARMX_AGENT_ID"],
            json!("claude-ccc")
        );
    }
    ...
    fn claude_stop_hook_merges_existing_user_hooks() {
        ...
        // Stop now has TWO entries: the user's (first) and swarmx (last).
        ...
        assert!(
            ...
            "swarmx entry appended at end: {cmd}"
        );
    }
