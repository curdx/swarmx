...
//! `~/.swarmx/opencode/<agent_id>.json`: the swarmx-swarm MCP server (with
...
//! so the user's provider/model survives and swarmx's keys win).
//!
...
impl CliAdapter for OpencodeAdapter {
    ...
    fn contribute_env(
        ...
        // user's config (it does NOT replace it) — swarmx's keys (swarm MCP w/
...
/// Per-agent opencode config file. Written under `~/.swarmx/opencode/` keyed
...
/// user's config — it does NOT replace it. swarmx's keys win on conflict, so
/// this file authoritatively sets the swarmx-swarm MCP server (with per-agent
...
/// process has its OWN OPENCODE_CONFIG file with its OWN agent_id, and swarmx
...
/// Merge the swarmx wake plugin into the per-agent opencode config's
/// `plugin[]`. opencode has no blocking Stop hook, so the wake loop runs as a
/// plugin: on `session.idle` it calls swarmx's `consume_wakes` and re-prompts
/// the session when wakes are pending (see `cli-plugins/opencode/swarmx-wake.js`
/// — the opencode equivalent of `swarmx-mcp wake-check`).
///
        ...
        None => {
            tracing::warn!(
                ...
                "opencode wake plugin JS not found; worker will have swarm_* tools but no \
                 auto-wake (set SWARMX_OPENCODE_PLUGIN, or ship cli-plugins/opencode/swarmx-wake.js)"
            );
            return Ok(());
        }
    };
...
/// Locate the bundled swarmx opencode wake plugin (a small JS file). Env
/// override first (the Tauri sidecar sets `SWARMX_OPENCODE_PLUGIN` to the
...
fn opencode_wake_plugin_path() -> Option<PathBuf> {
    if let Some(p) = std::env::var_os("SWARMX_OPENCODE_PLUGIN") {
        let p = PathBuf::from(p);
        if p.is_file() {
            return Some(p);
        }
    }
    // dev fallback: <workspace-root>/cli-plugins/opencode/swarmx-wake.js
    // (CARGO_MANIFEST_DIR is crates/swarmx-server; step up two levels).
...
mod tests {
    ...
    fn opencode_config_writes_mcp_permission_and_autoupdate() {
        ...
        assert_eq!(
            mcp["environment"]["SWARMX_AGENT_ID"],
            json!("opencode-abc123")
        );
        assert_eq!(
            mcp["environment"]["SWARMX_SERVER_URL"],
    ...
    fn opencode_plugin_merge_is_idempotent_and_preserves_mcp() {
        ...
        let plugin = dir.path().join("swarmx-wake.js");
        ...
        assert_eq!(v["permission"], json!("allow"));
        assert_eq!(v["mcp"]["swarmx-swarm"]["type"], json!("local"));
    }
