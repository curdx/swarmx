...
//! topology of a multi-agent swarmx session.
//!
...
pub fn default_spells_dir() -> PathBuf {
    if let Ok(p) = std::env::var("SWARMX_SPELLS_DIR") {
    ...
    WARN_ONCE.call_once(|| {
        tracing::warn!(
            "SWARMX_SPELLS_DIR unset and CARGO_MANIFEST_DIR-relative `spells/` not found; \
             falling back to CWD-relative `spells` — unreliable under the installed app (CWD=/). \
             Set SWARMX_SPELLS_DIR (Tauri sidecar) or rely on the embedded builtin spells."
        );
    });
    PathBuf::from("spells")
}
