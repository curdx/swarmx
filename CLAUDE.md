...
## 第二原则:改完之后必须用浏览器真实测一遍

`cargo test` / `tsc` / `curl` 只能证明代码能编译、端点有响应,**证明不了用户点下去是对的**。
凡是碰到前端、UI 交互、REST/SSE 端点、或任何影响页面行为的改动,落地前必须在浏览器里
**用 chrome-devtools 真实走一遍**:打开对应页面、点该点的按钮、看该出的结果——像真实用户那样操作,而不是只看后台日志或 curl 输出就宣称完成。

- 新功能:走通「用户从头到尾的完整路径」并截图留证(如:装引擎→点一键安装→看日志流→卡片翻已安装)。
...
cargo test  -p swarmx-swarm           # 单个 crate
cargo test  -p swarmx-mcp tools_list  # 单个测试(按名字过滤)
cargo clippy --workspace --all-targets  # lint(CI 里是 informational)
cargo fmt --all                         # 格式化
cargo run -p swarmx-server            # 起后端(必须从仓库根目录,才能就近找到 spells/roles/cli-plugins)
```

前端(`web/` 目录):

```bash
npm_config_cache=/tmp/.npm-swarmx npm install   # 装依赖(/tmp 缓存绕沙箱 EACCES)
...
1. **PTY 在最底层。** 每个 agent 是未经修改的真实 `claude`/`codex`/`opencode`/`reasonix` 二进制,
   跑在 `portable-pty` 下。OAuth、限流、套餐限制行为和你在终端里敲 `claude` 完全一致 —— swarmx **从不**
   读取或持久化 OAuth token,只把 `HOME` 透传给子进程,复用 `~/.claude/`、`~/.codex/` 里已有的凭证。
2. **shim 在中间。** `swarmx-shim`(`execvp` 真 CLI 的极薄包装)发 OSC ready/exit 序列,让服务端知道何时就绪/退出。
3. **MCP 在最上层。** swarm 消息以原生 MCP 工具(`swarm_send_message`、`swarm_write_blackboard`…)暴露给 LLM。
   `swarmx-mcp` 是个 stdio JSON-RPC server,同时托管 `wake-check` 子命令(被 CLI 的 Stop hook 调用)。
...
| `swarmx-cli` | 极薄入口(`swarmx up` 起 server + 开仪表盘) |
...
- 3 个 sidecar 二进制 `swarmx-server` / `swarmx-shim` / `swarmx-mcp` —— `externalBin`,由 `build-sidecar.sh` 拷
- `cli-plugins/opencode/swarmx-wake.js` —— **当前唯一**的 `bundle.resources` 项。它是 opencode 的 wake hook,
  是 JS、要交给 opencode/node 执行,**没法 embed**。运行时定位见 `cli/opencode.rs` 的 `opencode_wake_plugin_path()`:
  `SWARMX_OPENCODE_PLUGIN`(Tauri sidecar 注入打包后的绝对路径)> `CARGO_MANIFEST_DIR` 相对(仅 dev)。
...
- 前端依赖:`web/` 下 `npm_config_cache=/tmp/.npm-swarmx npm install`(绕沙箱 EACCES)。
- 端口:后端 7777 / 前端 5173。
- debug 构建下 Tauri **不**自动拉起服务,需自己 `cargo run -p swarmx-server`(从仓库根目录,才能就近找到资源目录)。
